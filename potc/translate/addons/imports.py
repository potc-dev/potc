...
_FROM_IMPORT_PATTERN = re.compile(
    r'^\s*from\s+(?P<from>[a-zA-Z0-9_\\.]+)\s+'
    r'import\s+(?P<import>[a-zA-Z0-9_]+)'
    r'(|\s+as\s+(?P<as>[a-zA-Z0-9_]+))\s*$'
)


def from_import_str(base, string):
    _match = _FROM_IMPORT_PATTERN.fullmatch(string)
    if _match:
        _dict = _match.groupdict()
        from_, import_, as_ = _dict.get('from'), _dict.get('import'), _dict.get('as', None)
        _current = base.from_(from_).import_(import_)
        if as_ is not None:
            _current = _current.as_(as_)
        return _current
    else:
        raise ValueError(f'Invalid from import - {repr(string)}.')


_DIRECT_IMPORT_PATTERN = re.compile(
    r'\s*import\s+(?P<import>[a-zA-Z0-9_\\.]+)'
    r'(|\s+as\s+(?P<as>[a-zA-Z0-9_]+))\s*$'
)


def direct_import_str(base, string):
    _match = _DIRECT_IMPORT_PATTERN.fullmatch(string)
    if _match:
        _dict = _match.groupdict()
        import_, as_ = _dict.get('import'), _dict.get('as', None)
        _current = base.import_(import_)
        if as_ is not None:
            _current = _current.as_(as_)
        return _current
    else:
        raise ValueError(f'Invalid direct import - {repr(string)}.')
